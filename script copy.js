...
// --- STATE DASHBOARD ---
let dashboardDateRange = { start: null, end: null }; // Untuk filter dashboard
...
async function fetchRecruitmentData(sheetName) {
  try {
    ...
    appData.recruitment[sheetName] = []; // Ensure it's an array on failure
  }
}
...
// --- FUNGSI BARU UNTUK MANPOWER PAGE ---
...
// --- FUNGSI UNTUK HALAMAN REKRUTMEN ---
function showRecruitmentTab(sheetName) {
  activeRecruitmentSheet = sheetName;
  selectedApplicant = null; // Reset pilihan saat ganti tab
  document.getElementById("process-selected-applicant-btn").disabled = true;
...
function renderRecruitmentTabPage(sheetName) {
  ...
  // Tambahkan checkbox "Pilih Semua" ke header
  thead.innerHTML = `<tr>
        <th class="p-3 font-semibold w-12 text-center">
            <input type="checkbox" onchange="toggleSelectAll(this, '${sheetName}')" class="rounded border-gray-300">
        </th>
        ${headers
          .map(
  ...
  if (paginatedData.length === 0) {
    tbody.innerHTML = `<tr><td colspan="${headers.length}" class="text-center p-8">Tidak ada data yang cocok dengan pencarian Anda.</td></tr>`;
  } else {
    tbody.innerHTML = paginatedData
      .map((row) => {
        const applicantId = row.ID_Pelamar;
        // Cek apakah ID ini ada di dalam Set pilihan
        const isSelected = selectedApplicants.has(applicantId);
        return `
                <tr class="border-b hover:bg-indigo-50 ${
                  isSelected ? "bg-indigo-100" : ""
                }">
                    <td class="p-3 text-center">
<input type="checkbox" onchange="toggleApplicantSelection(this, '${sheetName}', '${applicantId}')" 
...
function toggleApplicantSelection(checkbox, sheetName, applicantId) {
  // Setiap kali ada perubahan, pastikan sheet sumbernya sama.
  // Jika berbeda, reset pilihan.
  const currentSourceSheet = Array.from(selectedApplicants)[0]?.split("|")[0];
  if (currentSourceSheet && currentSourceSheet !== sheetName) {
    selectedApplicants.clear();
    // Uncheck semua checkbox lain yang terlihat
    document
      .querySelectorAll('#recruitment-table-body input[type="checkbox"]')
      .forEach((cb) => (cb.checked = false));
  }

  const uniqueId = `${sheetName}|${applicantId}`; // Gabungkan sheetName dan ID
  if (checkbox.checked) {
...
function toggleSelectAll(masterCheckbox, sheetName) {
  ...
  checkboxes.forEach((cb) => {
    ...
    // Dapatkan applicantId dari elemen checkbox (kita perlu menyimpannya di sana)
    const applicantId = cb.dataset.applicantId; // Kita perlu menambahkan ini di render
    if (applicantId) {
...
function updateProcessButtonState() {
  ...
  if (count > 0) {
    ...
    processBtn.querySelector(
      "span:last-child"
    ).textContent = `Proses ${count} Kandidat`;
  } else {
    processBtn.disabled = true;
    processBtn.querySelector("span:last-child").textContent = "Proses Kandidat";
  }
}
...
function openRecruitmentProcessModal() {
  ...
  const [sourceSheet, applicantId] = firstApplicantId.split("|");
...
async function submitApplicantProcessing() {
  ...
  try {
    // Kirim semua permintaan secara paralel
    ...
    await fetchRecruitmentData(targetSheet);
...
function setDashboardDateRange(rangeType) {
  ...
  switch (rangeType) {
    ...
    case "all_time":
    default:
      startDate = null; // null berarti tanpa batas awal
      break;
  }
  ...
  // Render ulang semua grafik di tab yang aktif
...
function renderPersonalDashboardCharts() {
  ...
  // Sekarang, gunakan 'filteredData' untuk memproses dan membuat grafik, bukan 'appData.manpower'
  // Filter data manpower berdasarkan rentang tanggal
  const filteredData =
    dashboardDateRange.start === null
      ? appData.manpower
      : appData.manpower.filter((emp) => {
          const hiringDate = new Date(emp["HIRING DATE"]);
          return (
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // --- EVENT LISTENER BARU UNTUK EXPORT ---
  document
    .getElementById("export-manpower-btn")
    .addEventListener("click", () => {
      // Kita ekspor data yang sudah terfilter sesuai tampilan pengguna
  ...
  document
    ...
    .addEventListener("click", () => {
      ...
      // Filter data sesuai pencarian di tab aktif
      const filteredData =
        state.searchTerm === ""
          ? data
          : data.filter((row) =>
...
/**
 * Mengekspor array of objects ke file Excel.
 * @param {Array<object>} data - Data yang akan diekspor.
 * @param {string} fileName - Nama file output (misal: 'laporan.xlsx').
 */
function exportToExcel(data, fileName) {
  ...
  // Menulis file dan memicu download
